...
class IBA01(MicroPyServer):
    ...
    def reset(self, args):
        ...
        :return:
        """
        pyb.LED(self.LED_RED).on()
        pyb.LED(self.LED_GREEN).on()
        pyb.LED(self.LED_YELLOW).on()
        pyb.LED(self.LED_BLUE).on()
        ...
        pyb.LED(self.LED_RED).off()
        pyb.LED(self.LED_GREEN).off()
        pyb.LED(self.LED_YELLOW).off()
        pyb.LED(self.LED_BLUE).off()
    ...
    async def _toggle_led(self, led, on_ms, off_ms, once=False):
    ...
    def led_toggle(self, args):
        ...
        if on_ms > 0:
            if thread_name not in self.ctx["threads"] or not self.ctx["threads"][thread_name]:
                ...
                self.loop.create_task(self._toggle_led(led, on_ms, off_ms, once))  # Schedule ASAP
